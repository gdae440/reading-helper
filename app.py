...
st.set_page_config(page_title="跟读助手 Pro (V9.6)", layout="wide", page_icon="🦋")
...
GTTS_LANG_MAP = {
    "🇬🇧 英语 (English)": "en", "🇫🇷 法语 (Français)": "fr", "🇩🇪 德语 (Deutsch)": "de",
    "🇪🇸 西班牙语 (Español)": "es", "🇷🇺 俄语 (Русский)": "ru", "🇯🇵 日语 (日本語)": "ja"
}
...
async def create_anki_package(selected_items):
    ...
    media_files = []; progress_bar = st.progress(0)
    for idx, item in enumerate(selected_items):
        lang = item.get('lang', '🇬🇧 英语 (English)'); voice = get_default_voice_for_lang(lang); audio_filename = f"audio_{random.randint(1000,9999)}_{idx}.mp3"
        try:
            communicate = edge_tts.Communicate(item['word'], voice); await communicate.save(audio_filename)
        except:
            try: g_lang = GTTS_LANG_MAP.get(lang, "en"); tts = gTTS(text=item['word'], lang=g_lang); tts.save(audio_filename)
            except: pass
        if os.path.exists(audio_filename):
            media_files.append(audio_filename)
            note = genanki.Note(model=my_model, fields=[f"{item['word']} <br> <small style='color:grey'>{item.get('ipa','')}</small>", f"🇨🇳 {item.get('zh','')} <br> 🇷🇺 {item.get('ru','')}", f"[sound:{audio_filename}]"])
            deck.add_note(note)
        progress_bar.progress((idx + 1) / len(selected_items))
    output_package = genanki.Package(deck); output_package.media_files = media_files
    pkg_bytes = io.BytesIO(); temp_pkg_name = "temp_output.apkg"; output_package.write_to_file(temp_pkg_name)
    ...
    progress_bar.empty(); return final_bytes

async def get_audio_bytes_memory(text, voice, rate_str, lang_choice):
    # 返回 (audio_bytes, error_msg, engine_name)
    try:
        communicate = edge_tts.Communicate(text, voice, rate=rate_str)
        mp3_fp = io.BytesIO()
        async for chunk in communicate.stream():
            if chunk["type"] == "audio": mp3_fp.write(chunk["data"])
        return mp3_fp.getvalue(), None, "Microsoft Edge (高质量)"
        ...
        try:
            ...
            return mp3_fp.getvalue(), f"Edge服务繁忙，已降级至Google引擎", "Google (备用)"
        except Exception as e_google:
            return None, f"失败: {e_edge} | {e_google}", "None"
...
def silicon_vocab_lookup_multilang(word, api_key, model_id):
    ...
    prompt = f"""You are a strictly formatted dictionary API. User input: "{word}". Task: 1. Detect language. 2. Provide IPA if English. 3. Translate to Chinese (zh) and Russian (ru). 4. Keep definitions concise. Return ONLY JSON: {{ "detected_lang": "...", "ipa": "...", "zh": "...", "ru": "..." }}"""
...
if 'audio_engine_used' not in st.session_state: st.session_state.audio_engine_used = "" # 记录当前引擎
...
with st.sidebar:
    ...
    with st.expander("🤖 模型配置", expanded=False):
        ...
        for k, v in [("chat_model", chat_model_input), ("ocr_model", ocr_model_input), ("trans_prompt", trans_prompt_input)]:
            if v != st.session_state.app_config.get(k): st.session_state.app_config[k] = v; save_config(st.session_state.app_config)

    st.divider()
    
    # 🔥 自动清理缓存逻辑：如果用户换了语言或声音，清除旧音频
    ...
    # 简单的 Session State 监听来实现清理
    if 'last_voice_id' not in st.session_state: st.session_state.last_voice_id = voice_id
    if st.session_state.last_voice_id != voice_id:
        st.session_state.audio_cache = None # 清空音频缓存
        st.session_state.audio_engine_used = "" # 清空引擎标记
        st.session_state.last_voice_id = voice_id

    if not api_input: st.warning("⚠️ 请输入 API Key"); st.stop()
...
with col1:
    ...
    with tab_txt:
        ...
        if st.button("确认文本"): st.session_state.current_text = txt; st.session_state.translation_result = ""; st.rerun()
    ...
    if st.session_state.current_text:
        ...
        with c_tts:
            if st.button(f"▶️ 播放语音 ({rate_str})", type="primary", use_container_width=True):
                with st.spinner("合成中..."):
                    # 🔥 调用双引擎
                    audio_bytes, msg, engine = asyncio.run(get_audio_bytes_memory(final_text, voice_id, rate_str, lang_choice))
                    
                    if audio_bytes:
                        st.session_state.audio_cache = audio_bytes
                        st.session_state.audio_engine_used = engine
                        if msg: st.toast(msg, icon="⚠️")
                        st.rerun()
                    else: st.error(f"失败: {msg}")
        
        if st.session_state.audio_cache:
            # 🔥 显示当前使用的引擎
            if "Microsoft" in st.session_state.audio_engine_used:
                st.caption(f"✅ 当前引擎: {st.session_state.audio_engine_used}")
            elif "Google" in st.session_state.audio_engine_used:
                st.caption(f"⚠️ 当前引擎: {st.session_state.audio_engine_used} (音色不可选)")
        ...
        with st.expander("🇨🇳 全文翻译 (DeepSeek)", expanded=False):
            ...
            if st.session_state.translation_result: st.info(st.session_state.translation_result)

with col2:
    st.subheader("📚 智能单词本")
    with st.form("lookup", clear_on_submit=True):
        c1, c2 = st.columns([3, 1])
        wq = c1.text_input("查词", label_visibility="collapsed", placeholder="输入单词...")
            ...
            if wq.strip():
                with st.spinner("查询中..."):
                    ...
                    if info:
                        ...
                        st.session_state.vocab_book.insert(0, {"word": wq, "lang": final_lang, "date": datetime.now().strftime("%Y-%m-%d"), "ipa": info.get("ipa", ""), "zh": info.get("zh", ""), "ru": info.get("ru", "")})
    ...
    if filtered_vocab:
        ...
        for d, items in grouped.items():
            ...
            for idx, item in enumerate(items):
                ...
                with c_chk:
                    if st.checkbox("", key=f"chk_{item['word']}_{d}_{idx}"): checked_items.append(item)
                with c_wd:
                    st.markdown(f"**{item['word']}**"); 
                    if item.get('ipa'): st.caption(f"[{item['ipa']}]")
                    if st.button("🔊", key=f"p_{item['word']}_{d}_{idx}"):
                        adata, _, _ = asyncio.run(get_audio_bytes_memory(item['word'], voice_id, "+0%", lang_choice))
                        if adata: st.session_state.temp_word_audio[item['word']] = adata; st.rerun()
                with c_ph: st.markdown(f"🇨🇳 {item.get('zh','')}"); st.markdown(f"🇷🇺 {item.get('ru','')}")
                if item['word'] in st.session_state.temp_word_audio: st.audio(st.session_state.temp_word_audio[item['word']], format="audio/mpeg", autoplay=True); del st.session_state.temp_word_audio[item['word']]
            st.divider()
        if checked_items:
            col_exp, col_del = st.columns(2)
            with col_exp:
                if st.button("📤 导出Anki包"):
                    with st.spinner("打包中..."): apkg_bytes = asyncio.run(create_anki_package(checked_items)); st.download_button("⬇️ 下载 .apkg", data=apkg_bytes, file_name=f"anki_{datetime.now().strftime('%m%d')}.apkg")
            with col_del:
                if st.button("🗑️ 删除选中"): rem_words = [i['word'] for i in checked_items]; st.session_state.vocab_book = [i for i in st.session_state.vocab_book if i['word'] not in rem_words]; save_vocab(st.session_state.vocab_book); st.rerun()
    else: st.caption(f"暂无 {lang_choice} 生词")